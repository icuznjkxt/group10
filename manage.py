...
from donkeycar.parts.module1 import MODULE1
...
def drive(cfg, model_path=None, use_joystick=False, model_type=None, camera_type='single', meta=[] ):
    ...
    #'''
     # module1:DHT11 and beep
    module1 = MODULE1(17, 4)
    V.add(module1, outputs=['temp', 'humidity'], threaded=True)
    #'''
    ...
    class MySwitch:
        ...
        def audio(self,message):
            ...
            # 获取token
            self.apiUrl = 'https://aip.baidubce.com/oauth/2.0/token'
            self.data = {
                'grant_type': 'client_credentials',
                'client_id': 'vAwZIfgFaoBgq5TMqM6UQ9uK',
                'client_secret': 'CLkrmUSBBm6S4RS3GYWt4xUGmTHOaMBK',  # Secret Key
            }
            try:
                self.r = requests.get(self.apiUrl, params=self.data).json()
                print(r.get("access_token"))
            except Exception as e:
                print(e)

            self.apiUrl = 'https://aip.baidubce.com/rpc/2.0/ai_custom/v1/sound_cls/trysix'
            self.filename = "output.wav"
            self.file1 = open(self.filename, "rb").read()
            self.text = base64.b64encode(self.file1).decode("utf-8")
            self.data = {

                "sound": self.text,
            }
            self.apiUrl = self.apiUrl + "?access_token=" + self.r.get("access_token")

            self.encodedata = json.dumps(self.data).encode('utf-8')
            #print(encodedata)
            self.request = urllib.request.Request(self.apiUrl, self.encodedata)
            ...
            if (self.content):
                #print(self.content)
                self.user_dict = json.loads(self.content)
                #print(self.user_dict)
                print(self.user_dict.get("results")[0]['name'])
                if self.user_dict.get("results")[0]['name'] is not None:
                    print(self.user_dict.get("results")[0]['name'])
                    self.final = self.user_dict.get("results")[0]['name']
                else:
                    self.final = 'nothing'
            return self.final
        ...
        def run(self):
            ...
            check = self.audio("Starting every 10 seconds.")
            #time.sleep(25)
            ...
            if self.num == 0:
                return 1
            elif self.num >=1 and self.num <10:
                return 2
            else:
                return 3
    ...
    V.add(Stop(), inputs=[], outputs=['stop/angle','stop/throttle'], threaded=True)
    ...
    V.add(MyPilot(), inputs=[], outputs=['mypilot/angle','mypilot/throttle'], threaded=True)
    '''
    ...
    class ForwardBack():
        ...
        def run_threaded(self, img_arr=None):
            ...
            if self.distance > 40:
                if self.num >= 0 and self.num < 30:
                        self.throttle = 0.35
                        self.num = self.num + 1
                elif self.num >= 30 and self.num < 50:
                    self.throttle = 0.0
                    self.num = self.num + 1
                elif self.num >= 50 and self.num < 60:
                    self.throttle = -0.1
                    self.num = self.num + 1
                elif self.num >= 60 and self.num < 75:
                    self.throttle = -0.2
                    self.num = self.num + 1
                elif self.num >= 75 and self.num < 90:
                    self.throttle = -0.3
                    self.num = self.num + 1
                elif self.num >= 90 and self.num < 100:
                    self.throttle = -0.4
                    self.num = self.num + 1
                elif self.num >= 100 and self.num < 140:
                    self.throttle = -0.45
                    self.num = self.num + 1
                elif self.num >= 140 and self.num < 170:
                    self.throttle = 0.0
                    self.num = self.num + 1
                elif self.num == 170:
                    self.throttle = 0.0
    ...
    V.add(ForwardBack(pi,cfg.RANGE_GPIOS), inputs=[], outputs=['mypilot/angle', 'mypilot/throttle'], threaded=True)
    ...
    class DriveMode:
        def run(self, mode,
                    ...
                    stop_angle, stop_throttle,run_mypilot):
            if mode == 'user':
                print(user_angle, user_throttle)
                return user_angle, user_throttle
            elif mode == 'local_angle':
                print(pilot_angle, user_throttle)
                return pilot_angle, user_throttle
            else:
                if (run_mypilot == 1):
                    return stop_angle, stop_throttle
                elif (run_mypilot == 2):
    ...
    V.add(DriveMode(),
          inputs=['user/mode', 'user/angle', 'user/throttle', 'pilot/angle', 'pilot/throttle',
                  'mypilot/angle', 'mypilot/throttle', 'stop/angle', 'stop/throttle', 'run_mypilot'],
